...
_friedman() {
  ...
  case "${words[1]}" in
    ...
    forecast) COMPREPLY=( $(compgen -W "aparch arch arima bvar cgarch dynamic egarch evaluate favar fiegarch figarch garch garch-midas gdfm gjr-garch igarch lp setar star static sv var vecm" -- "$cur") ) ;;
    ...
    predict) COMPREPLY=( $(compgen -W "aparch arch arima bvar cgarch dynamic egarch favar fiegarch figarch garch garch-midas gdfm gjr-garch igarch logit mlogit ologit oprobit piv plogit pprobit preg probit reg static sv var vecm" -- "$cur") ) ;;
    residuals) COMPREPLY=( $(compgen -W "aparch arch arima bvar cgarch dynamic egarch favar fiegarch figarch garch garch-midas gdfm gjr-garch igarch logit mlogit ologit oprobit piv plogit pprobit preg probit reg static sv var vecm" -- "$cur") ) ;;
    spectral) COMPREPLY=( $(compgen -W "acf cross density periodogram transfer" -- "$cur") ) ;;
    test) COMPREPLY=( $(compgen -W "adf adf-2break andrews arch-lm ardl-bounds bai-perron bartlett-wn bds box-pierce brant breitung breusch-pagan chow cips cusum cusumsq dfgls dh-causality durbin-watson edf engle-granger ers f-fe factor-break fisher fisher-johansen fourier-adf fourier-kpss glejser gph granger gregory-hansen gsadf hadri hansen-instability hansen-linearity harvey hausman hausman-iia hegy heteroskedasticity identifiability influence ips johansen kao kpss ljung-box llc lm lm-unitroot local-whittle lr modified-wald moon-perron nardl-symmetry normality np nyblom panic park-added pedroni pesaran-cd phillips-ouliaris pmg-hausman pp pvar recursive-residuals sadf sign-bias star-linearity var variance-ratio vecm vif weak-instrument westerlund white wooldridge-ar za" -- "$cur") ) ;;
  esac
}
