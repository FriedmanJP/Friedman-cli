...
_friedman() {
  ...
  case "${words[1]}" in
    ...
    estimate) COMPREPLY=( $(compgen -W "choice factor panel regime regression univariate var volatility" -- "$cur") ) ;;
    fevd) COMPREPLY=( $(compgen -W "bvar favar lp pvar sdfm var vecm" -- "$cur") ) ;;
    filter) COMPREPLY=( $(compgen -W "bhp bk bn hamilton hp x13" -- "$cur") ) ;;
    forecast) COMPREPLY=( $(compgen -W "evaluate factor regime univariate var volatility" -- "$cur") ) ;;
    ...
    predict) COMPREPLY=( $(compgen -W "choice factor panel regime regression univariate var volatility" -- "$cur") ) ;;
    residuals) COMPREPLY=( $(compgen -W "choice factor panel regime regression univariate var volatility" -- "$cur") ) ;;
    ...
    test) COMPREPLY=( $(compgen -W "brant coint did dispersion edf fisher gph hansen-linearity hausman-iia identifiability influence iv local-whittle nardl-symmetry normality panel park-added pvar serial stability star-linearity unit-root var variance-ratio vecm vif" -- "$cur") ) ;;
  esac
}
