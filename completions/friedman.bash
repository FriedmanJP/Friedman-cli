...
_friedman() {
  ...
  case "${words[1]}" in
    ...
    test) COMPREPLY=( $(compgen -W "adf adf-2break andrews arch-lm ardl-bounds bai-perron bartlett-wn bds box-pierce brant breusch-pagan cips dfgls durbin-watson edf engle-granger ers f-fe factor-break fisher fourier-adf fourier-kpss gph granger gregory-hansen gsadf hadri hansen-instability hansen-linearity hausman hausman-iia hegy heteroskedasticity identifiability johansen kao kpss ljung-box lm lm-unitroot local-whittle lr modified-wald moon-perron nardl-symmetry normality np nyblom panic park-added pedroni pesaran-cd phillips-ouliaris pmg-hausman pp pvar sadf sign-bias star-linearity var variance-ratio vecm vif weak-instrument westerlund wooldridge-ar za" -- "$cur") ) ;;
  esac
}
