...
@testset "subprocess E2E contract (T4)" begin
    ...
    @testset "snapshots --version / --help / schema" begin
        ...
        r = _e2e_run(["schema", "estimate", "var", "var"])
        @test r.code == 0
    ...
    @testset "exit codes 2/3/4/6" begin
        ...
        r = _e2e_run(["estimate", "var", "var", "d.csv", "--lgas", "2"])
        @test r.code == 2

        # data → 3
        r = _e2e_run(["estimate", "var", "var", "/nope/does/not/exist.csv"])
        @test r.code == 3

        # config → 4 (missing config file with --strict optional; missing file is config class)
        r = _e2e_run(["estimate", "var", "bvar", "/nope.csv", "--config", "/nope/config.toml"])
    ...
    @testset "stdout purity — one JSON across top-level families" begin
        ...
        try
            ...
            cases = [
                ["--quiet", "estimate", "var", "var", fix, "--lags", "1", "--format", "json"],
                ...
                ["--quiet", "forecast", "var", "var", fix, "--lags", "1", "--horizons", "2", "--format", "json"],
                ["--quiet", "predict", "var", "var", fix, "--lags", "1", "--format", "json"],
                ["--quiet", "residuals", "var", "var", fix, "--lags", "1", "--format", "json"],
    ...
    @testset "no-color / CRLF safety" begin
        fix = _make_fixture()
        try
            r = _e2e_run(["--no-color", "--quiet", "estimate", "var", "var", fix, "--lags", "1", "--format", "table"])
            @test r.code == 0
            ...
            r2 = _e2e_run(["--no-color", "--quiet", "estimate", "var", "var", fix, "--lags", "1", "--format", "json"])
            @test r2.code == 0
    ...
    @testset "quiet + json purity seed cases" begin
        fix = _make_fixture()
        try
            r = _e2e_run(["--quiet", "estimate", "var", "var", fix, "--lags", "1", "--format", "json"])
            @test r.code == 0
            @test _exactly_one_json(r.out)
    ...
    @testset "error envelopes when JSON was asked (W2/#137)" begin
        ...
        try
            ...
            r = _e2e_run(["estimate", "var", "var", "/nope/does/not/exist.csv", "--format", "json"])
            @test r.code == 3
            @test _exactly_one_json(r.out)
            doc = JSON3.read(strip(r.out))
            ...
            r = _e2e_run(["estimate", "var", "var", fix, "--lgas", "2", "--format", "json"])
            @test r.code == 2
            @test _exactly_one_json(r.out)
            doc = JSON3.read(strip(r.out))
            @test string(doc.status) == "error"
            ...
            @test string(doc.command) == "friedman estimate var var"
