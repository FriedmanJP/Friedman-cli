...
@testset "real entry points (build_app / run_cli)" begin
    ...
    @testset "run_cli exit classes" begin
        ...
        # data
        @test Int(Friedman.run_cli(["estimate", "multivariate", "var", "/nonexistent/file.csv"])) == 3
    end
    ...
    @testset "pre-dispatch globals are leading-only (#117)" begin
        ...
        redirect_stdout(devnull) do
            redirect_stderr(devnull) do
                @test Int(Friedman.run_cli(["estimate", "multivariate", "var", "nofile.csv", "--warranty"])) == 2
                @test Int(Friedman.run_cli(["estimate", "multivariate", "var", "nofile.csv", "--conditions"])) == 2
                @test Int(Friedman.run_cli(["estimate", "multivariate", "var", "nofile.csv", "--version"])) == 2
