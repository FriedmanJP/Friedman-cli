...
@testset "estimate smm command structure" begin
    ...
    # 61 primary leaves + 1 snake alias (gjr_garch) = 62 keys (C044; +6 GARCH variants C064a, +arfima C068, +3 MGARCH C064b, +5 penalized/robust/tobit C067a, +truncreg/heckman C067b, +5 statespace/tvp/kde/kernel-reg/lowess C066, +cointreg/xtcointreg C062a, +ardl/nardl C062b, +pmg C062c, +midas C062d)
    @test length(est_node.subcmds) == 62
    @test haskey(est_node.subcmds, "smm")
...
@testset "Structural break test command structure" begin
    ...
    # 56 primary + 2 snake aliases (C044; +gph, +local-whittle C068, +sign-bias, +nyblom C064b, +vecm C071, +variance-ratio/bds/hadri/pedroni/kao/westerlund C069/C070, +weak-instrument C067b, +ardl-bounds/nardl-symmetry C062b, +pmg-hausman C062c)
    @test length(test_node.subcmds) == 58
