...
>
> **v0.13.0 (typed handles; MEMs 0.9.6):** targets MacroEconometricModels **v0.9.6** from the Julia General registry. Pin rule: CI/release/integration resolve against an **exact** pin, `[compat] MacroEconometricModels = "=0.9.6"` as of CLI v0.13.0 (canary/nightly-dev may track MEMs `dev`). The pin is exact because the root `Manifest.toml` is gitignored, so a release re-resolves deps — a caret range once floated an unvetted patch release into a shipped build. At this baseline JuMP + Ipopt remain required upstream deps (bundled — see the solver note below).
...
As of CLI v0.13.0 (MacroEconometricModels 0.9.6) 145 of 456 leaves are plot-capable (`friedman show` plots when a recipe exists).
