...
|---------|-------------|-------------|
| `estimate` | `var` `bvar` `lp` `arima` `arfima` `gmm` `smm` `static` `dynamic` `gdfm` `arch` `garch` `egarch` `gjr-garch` `sv` `igarch` `cgarch` `aparch` `figarch` `fiegarch` `garch-midas` `ccc` `dcc` `bekk` `lasso` `ridge` `elastic-net` `robust` `tobit` `truncreg` `heckman` `fastica` `ml` `vecm` `pvar` `favar` `sdfm` `reg` `iv` `sur` `3sls` `logit` `probit` `ologit` `oprobit` `mlogit` `preg` `pols` `pfe` `pre` | Estimate models (50 model types, incl. ARFIMA long memory, 6 univariate GARCH variants, multivariate GARCH CCC/DCC/BEKK, penalized (Lasso/Ridge/Elastic-Net) + robust + Tobit + truncated + Heckman sample-selection regression, SUR/3SLS systems, panel reg, ordered/multinomial choice) |
