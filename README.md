...
|---------|-------------|-------------|
| `estimate` | `var` `bvar` `lp` `arima` `arfima` `gmm` `smm` `static` `dynamic` `gdfm` `arch` `garch` `egarch` `gjr-garch` `sv` `igarch` `cgarch` `aparch` `figarch` `fiegarch` `garch-midas` `ccc` `dcc` `bekk` `fastica` `ml` `vecm` `pvar` `favar` `sdfm` `reg` `iv` `sur` `3sls` `logit` `probit` `ologit` `oprobit` `mlogit` `preg` `pols` `pfe` `pre` | Estimate models (43 model types, incl. ARFIMA long memory, 6 univariate GARCH variants, multivariate GARCH CCC/DCC/BEKK, SUR/3SLS systems, panel reg, ordered/multinomial choice) |
