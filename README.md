...
| `forecast` | `var` `bvar` `lp` `arima` `setar` `static` `dynamic` `gdfm` `arch` `garch` `egarch` `gjr-garch` `sv` `vecm` `favar` · `evaluate metrics/dm/clark-west/mincer-zarnowitz/encompassing/combine` | Forecasting (15 model types, incl. SETAR bootstrap-simulation forecasts) + evaluation & combination sub-family |
