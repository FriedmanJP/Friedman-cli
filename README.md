...
>
> **v0.12.1 (MEMs 0.9.4 adoption):** targets MacroEconometricModels **v0.9.4** from the Julia General registry. Pin rule: CI/release/integration resolve against an **exact** pin, `[compat] MacroEconometricModels = "=0.9.4"` as of CLI v0.12.1 (canary/nightly-dev may track MEMs `dev`). The pin is exact because the root `Manifest.toml` is gitignored, so a release re-resolves deps — a caret range once floated an unvetted patch release into a shipped build. At this baseline JuMP + Ipopt become required upstream deps (bundled — see the solver note below).
...
As of CLI v0.12.1 (MacroEconometricModels 0.9.4) 144 of 453 leaves are plot-capable.
