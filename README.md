...
friedman schema estimate var var | jq '.tables'         # stable result-table keys
...
- **Envelope schema v1 is frozen at v1.0.0**: within
  `schema_version: 1` no key is removed or retyped; a breaking envelope change
  bumps `schema_version` to 2 and is a **major** release.
- **Minors are additive-only**: new commands, options, tables, and error
...
- **Removed in v1.0.0**: the hidden snake_case aliases and
  `FRIEDMAN_LEGACY_OUTPUT` (v1.0.0 freeze; kebab-case only, envelope JSON only).
...
The installer checks for Julia 1.13 (installs [juliaup](https://github.com/JuliaLang/juliaup) if needed, without changing your default Julia version), downloads a precompiled sysimage, and installs to `~/.friedman-cli/`.
...
|---------|-------------|-------------|
| `estimate` | `var` · `volatility` · `factor` · `univariate` · `regime` · `panel` · `choice` · `regression` — path is `estimate <family> <model>` (`estimate var var`, `estimate volatility garch`, `estimate panel preg`) | Estimate models (ARFIMA long memory, univariate and multivariate GARCH, penalized and robust regression, Tobit, truncated, and Heckman selection, FMOLS/CCR/DOLS and panel cointegration, ARDL and NARDL with folded multipliers, PMG/MG/DFE, MIDAS, threshold/SETAR/STAR, Markov-switching, state-space and TVP, nonparametric regression, SUR/3SLS, panel regression, ordered and multinomial choice) |
| `test` | `unit-root` · `coint` · `stability` · `serial` · `iv` · `panel` · `var` · `vecm` · `pvar` · `did`, plus flat leaves such as `vif` and `hausman-iia` (`test unit-root adf`, `test panel pmg-hausman`, `test vif`; `test other …` is not a command) | Statistical tests (unit-root and cointegration batteries, stability and serial diagnostics, weak-instrument tests, panel specification and DiD diagnostics, VAR/VECM/PVAR tests, and the flat diagnostics that stay directly under `test`) |
...
| `forecast` | `var` · `volatility` · `factor` · `univariate` · `regime` · `evaluate` (`forecast var var`, `forecast volatility gjr-garch`, `forecast var scenario`, `forecast evaluate metrics`) | Forecasting, including SETAR/STAR bootstrap-simulation forecasts, plus the evaluation and combination leaves under `forecast evaluate` |
| `predict` | `var` · `volatility` · `factor` · `univariate` · `regime` · `panel` · `choice` · `regression` (`predict var var`, `predict choice poisson`; no `setar` or `star`) | In-sample fitted values (`statespace` emits the filtered/smoothed state paths; `sur`/`3sls` one long per-equation table) |
...
| `data` | `list` `load` `import` `export` `describe` `diagnose` `fix` `transform` `filter` `validate` `balance` `dropna` `keeprows` `simulate <dgp>` | Data management (typed handles, example datasets, diagnostics, simulation) |
| `io` | `sources` `download` `load` `leontief` `ghosh` `multipliers` `linkages` `key-sectors` `sda` `extract` `footprint` `baqaee-farhi` `bf <sub>` `aggregate` `balance` `bilateral-trade` `export-decomposition` `impact` `network-stats` `price` `vertical-specialization` | Input-output analysis (multipliers, linkages, SDA, footprints, Baqaee-Farhi) |
| `nowcast` | `dfm` `bvar` `bridge` `news` `forecast` | Nowcasting (DFM, BVAR, bridge equations) |
| `dsge` | RA leaves + `bayes` + `ct` + `olg` + `dcegm` + `lifecycle` + `firm` + `bank` | Representative-agent DSGE, Bayesian workflow, and closed agent families |
| `hadsge` | `solve` `steady-state` `irf` `fevd` `simulate` `hd` `estimate` `accuracy` `distribution-irf` `inequality-irf` `simulate-panel` | One-household heterogeneous-agent DSGE (SSJ, Reiter, Krusell–Smith) |
...
| `did` | `estimate` `event-study` `lp-did` | Difference-in-differences estimation (Bacon / pretrend / negweight / honest are `test did`) |
| `policy` | `effects` (`var` `bvar` `lp` `sign`) + `counterfactual` (`var` `bvar` `lp`) + `optimal` (`var` `bvar` `lp`) + `moments` (`var` `bvar`) + `opp` / `opp-sequence` (`var` `bvar`) + `history` (`var` `bvar`) + `news` (`dsge` `ha`) + `jacobian` (`ha`) + `spanning` (`var`) + `sufficiency` (`dsge`) | Policy counterfactuals & optimal policy (McKay-Wolf causal-effect menus, Barnichon-Mesters OPP, rule counterfactuals, structural news/Jacobian routes) |
...
| `model` | `info` `reproduce` | Saved-handle inspection (`info`) and manifest-driven reproduction (`reproduce`) |
| `show` | — | Render any loadable handle stem (data, model, or result bundle) |
...
friedman estimate var bvar data.csv --lags=4 --draws=2000 --sampler=nuts

# Bayesian VAR with Minnesota prior config
friedman estimate var bvar data.csv --config=prior.toml

# Bayesian posterior summary (mean or median)
friedman estimate var bvar data.csv --lags=4 --method=mean
...
friedman estimate univariate arima data.csv --criterion=bic   # auto-select
...
friedman estimate var favar data.csv --lags=4 --nfactors=3
friedman estimate var favar data.csv --lags=4 --nfactors=3 --slow-vars=1,2,3

# Structural Dynamic Factor Model (SDFM)
friedman estimate factor sdfm data.csv --nfactors=3 --factor-lags=2
friedman estimate factor sdfm data.csv --nfactors=3 --id=cholesky
```
...
friedman estimate regression reg data.csv --dep=wage --weights=pop_weight
...
# Probit
friedman estimate choice probit data.csv --dep=employed --clusters=state

# Predictions with marginal effects
friedman predict choice logit data.csv --dep=employed --marginal-effects
...
friedman estimate univariate nardl data.csv --dep=y --horizon=24 --nreps=500
```
...
friedman estimate panel pmg panel.csv --dep=y --indep=x1,x2 --method=dfe

# PMG Hausman selection test (H0: long-run homogeneity; low p favours MG)
friedman test panel pmg-hausman panel.csv --dep=y --indep=x1,x2 --efficient=pmg
```
...
friedman estimate regime setar y.csv --p 2 --d auto
...
friedman estimate regime star y.csv --p 2 --type lstr1
...
friedman estimate regime ms-ar y.csv --p 1 --k-regimes 2
# ... with per-regime (switching) variances
friedman estimate regime ms-ar y.csv --p 1 --switching-variance
...
friedman estimate volatility sv data.csv --column=1 --draws=2000
...
friedman test stability nyblom data.csv --column=1 --model=gjr-garch
```
...
friedman test unit-root np data.csv --column=1
...
friedman test serial arch-lm data.csv --lags=4
friedman test serial ljung-box data.csv --lags=10
...
friedman test var lr data.csv data.csv --lags1=2 --lags2=4
friedman test var lm data.csv data.csv --lags1=2 --lags2=4
...
friedman test stability andrews data.csv --column=1 --trim=0.15
friedman test stability bai-perron data.csv --column=1 --max-breaks=5

# Panel unit root tests
friedman test panel panic data.csv --id-col=country --time-col=year --column=gdp
friedman test unit-root cips data.csv --id-col=country --time-col=year --column=gdp --lags=1
friedman test unit-root moon-perron data.csv --id-col=country --time-col=year --column=gdp
friedman test stability factor-break data.csv --id-col=country --time-col=year --column=gdp
...
friedman test unit-root lm-unitroot data.csv --column=1 --breaks=1 --regression=level
friedman test unit-root adf-2break data.csv --column=1 --model=level --trim=0.10
friedman test coint gregory-hansen data.csv --model=C --lags=aic
...
friedman forecast var var data.csv --horizons=12 --ci=bootstrap --replications=500
...
friedman forecast factor static data.csv --horizon=12
friedman forecast factor dynamic data.csv --nfactors=2 --factor-lags=1 --horizon=12
friedman forecast factor gdfm data.csv --dynamic-rank=2 --horizon=12

# Volatility model forecasting
friedman forecast volatility arch data.csv --column=1 --horizons=12
friedman forecast volatility garch data.csv --column=1 --horizons=12
friedman forecast volatility egarch data.csv --column=1 --horizons=12
friedman forecast volatility gjr-garch data.csv --column=1 --horizons=12
friedman forecast volatility sv data.csv --column=1 --horizons=12

# VECM forecast (bootstrap CIs)
friedman forecast var vecm data.csv --horizons=12 --rank=2

# FAVAR forecast
friedman forecast var favar data.csv --horizons=12 --nfactors=3
```
...
friedman predict var bvar data.csv --lags=4 --draws=2000
friedman predict univariate arima data.csv --p=1 --d=1 --q=1
friedman predict var vecm data.csv --rank=1
friedman predict factor static data.csv --nfactors=3
friedman predict volatility garch data.csv --column=1 --p=1 --q=1
friedman predict var favar data.csv --lags=4 --nfactors=3
friedman predict regression reg data.csv --dep=wage
friedman predict choice logit data.csv --dep=employed --marginal-effects
friedman predict choice probit data.csv --dep=employed --classification-table

# Model residuals
friedman residuals var var data.csv --lags=2
friedman residuals var bvar data.csv --lags=4 --draws=2000
friedman residuals univariate arima data.csv --p=1 --d=1 --q=1
friedman residuals var vecm data.csv --rank=1
friedman residuals factor static data.csv --nfactors=3
friedman residuals volatility garch data.csv --column=1 --p=1 --q=1
friedman residuals var favar data.csv --lags=4 --nfactors=3
friedman residuals regression reg data.csv --dep=wage
friedman residuals choice logit data.csv --dep=employed
friedman residuals choice probit data.csv --dep=employed
```
...
# HA-DSGE builtins: huggett | krusell-smith | one-asset-hank | two-asset-hank
...
friedman hadsge inequality-irf huggett --method=reiter
...
`forecast` drops `shock`). Coefficient-bearing models (`estimate var var`, `estimate regression reg`, `estimate regression iv`, `estimate choice logit`,
`estimate choice probit`, and the panel, ordered, and multinomial leaves) render through `DataFrame(model)`: one row per term,
...
`data/wrong-kind` (exit 3) — e.g. a panel handle on `estimate var var`. A
...
- **`.jld2`** — native MacroEconometricModels.jl `save_model`/`load_model` (JLD2-backed,
  ...
  registry — **352 types** at MacroEconometricModels 1.0.0: everything above plus SVAR
...
`estimate regime star`), and the same holds for Markov-switching forecasts — `estimate regime ms` / `estimate regime ms-ar` plot, `forecast regime ms` / `forecast regime ms-ar` do not.
