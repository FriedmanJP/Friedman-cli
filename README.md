...
| `residuals` | `3sls` `aparch` `arch` `arfima` `arima` `bvar` `cgarch` `dynamic` `egarch` `favar` `fiegarch` `figarch` `garch` `garch-midas` `gjr-garch` `igarch` `logit` `mlogit` `ologit` `oprobit` `piv` `plogit` `pprobit` `preg` `probit` `reg` `statespace` `static` `sur` `sv` `var` `vecm` | Model residuals (32 model types; `statespace` emits Kalman innovations, `--standardized` for `v_t/√F_t`) |
