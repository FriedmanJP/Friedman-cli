...
├── estimate     3sls | aparch | arch | arfima | arima | bekk | bvar | ccc | cgarch | dcc | dynamic | egarch | elastic-net | fastica | favar | fiegarch | figarch | garch | garch-midas | gdfm | gjr-garch | gmm | heckman | igarch | iv | kde | kernel-reg | lasso | logit | lowess | lp | ml | mlogit | ologit | oprobit | piv | plogit | pprobit | preg | probit | pvar | reg | ridge | robust | sdfm | smm | statespace | static | sur | sv | tobit | truncreg | tvp | var | vecm
...
Total: 17 top-level commands, 280 leaves (from registry).
```
...
- [`estimate`](generated/estimate.md) — 55 leaves
