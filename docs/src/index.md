...
| **Policy Counterfactuals** | McKay-Wolf causal-effect menus, rule counterfactuals, optimal policy, second moments | `policy effects`, `policy counterfactual`, `policy optimal`, `policy moments` |
...
**19 top-level commands, 399 subcommands.** Action-first CLI: commands organized by action (`estimate`, `irf`, `forecast`, `did`, `policy`, ...) rather than by model type.
